...
export function registerLeaderboardRoutes(app: Express) {
  ...
  app.get('/leaderboard/picks', async (_req, res: Response) => {
    try {
      const supabase = getServiceClient()
      const [authUsersRes, settingsRes, picksRes] = await Promise.all([
        supabase.auth.admin.listUsers({ page: 1, perPage: 1000 }),
        supabase.from('user_settings').select('user_id,display_name,avatar_url'),
        supabase.from('user_daily_picks').select('user_id,player_id,pick_date,hit'),
      ])

      const authUsers = authUsersRes.data?.users ?? []
      const settings = (settingsRes.data ?? []) as Array<{ user_id: string; display_name: string | null; avatar_url: string | null }>
      const picks = (picksRes.data ?? []) as Array<{ user_id: string; player_id: string; pick_date: string; hit: boolean | null }>
      const playerIds = [...new Set(picks.map((pick) => Number(pick.player_id)).filter((n) => Number.isFinite(n) && n > 0))]
      const dates = [...new Set(picks.map((pick) => String(pick.pick_date)).filter(Boolean))]
      const { data: statRowsRes } =
        playerIds.length && dates.length
          ? await supabase
              .from('player_stats_daily')
              .select('player_id,date,home_runs')
              .in('player_id', playerIds)
              .in('date', dates)
          : { data: [] as Array<{ player_id: number | string; date: string; home_runs: number | null }> }
      const statRows = (statRowsRes ?? []) as Array<{ player_id: number | string; date: string; home_runs: number | null }>

      const settingsByUser = new Map(settings.map((row) => [row.user_id, row]))
      const hitSet = new Set(
        statRows
          .filter((row) => Number(row.home_runs ?? 0) > 0)
          .map((row) => `${String(row.player_id)}|${String(row.date)}`),
      )

      const userIds = new Set<string>()
      for (const user of authUsers) userIds.add(user.id)
      for (const row of settings) userIds.add(row.user_id)
      for (const row of picks) userIds.add(row.user_id)

      const rows = [...userIds].map((userId) => {
        const authUser = authUsers.find((user) => user.id === userId) ?? null
        const settingsRow = settingsByUser.get(userId) ?? null
        const userPicks = picks.filter((pick) => pick.user_id === userId)
        let hits = 0
        for (const pick of userPicks) {
          if (pick.hit === true) hits += 1
          else if (pick.hit === false) continue
          else if (hitSet.has(`${pick.player_id}|${pick.pick_date}`)) hits += 1
        }
        const totalPicks = userPicks.length
        const hitPct = totalPicks ? Number(((hits / totalPicks) * 100).toFixed(2)) : 0
        return {
          user_id: userId,
          display_name: settingsRow?.display_name?.trim() || authUser?.email?.split('@')[0] || 'User',
          avatar_url: settingsRow?.avatar_url ?? null,
          hits,
      ...
      rows.sort((a, b) => {
        if (b.hit_pct !== a.hit_pct) return b.hit_pct - a.hit_pct
        if (b.total_picks !== a.total_picks) return b.total_picks - a.total_picks
        return a.display_name.localeCompare(b.display_name)
      })

      res.json({ rows })
    } catch (e) {
      console.error('[leaderboard/picks] failed:', e)
      res.status(500).json({ error: e instanceof Error ? (e.stack ?? e.message) : String(e) })
    }
  })
}
